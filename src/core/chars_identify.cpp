...
////return the result char's index. just one char indentification
...
std::pair<int, float> CCharsIdentify::tcl_classify(Mat f, bool isChinses, bool isSpeci) {
  int result = -1;
  Mat output(1, numAll, CV_32FC1);
  ann.predict(f, output);  //使用ann对字符做判断
  ...
  return std::make_pair(result, maxVal);
}
...
string CCharsIdentify::tcl_charsIdentify(Mat input, bool isChinese, 
                                         bool isSpeci, float &matchVal) {
  Mat f = features(input, m_predictSize);

  string result = "";
  //get the index of char and its match value
  std::pair<int, float> indexAndVal = tcl_classify(f, isChinese, isSpeci);
  int index = indexAndVal.first;
  matchVal = indexAndVal.second;
